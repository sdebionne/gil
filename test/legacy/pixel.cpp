...
struct increment
{
    ...
    void operator()(Incrementable& x) const
    {
        ++x;
    }
};
    ...
    , public C2
{
    ...
    void test_heterogeneous()
    {
        ...
        boost::function_requires<
            PixelsCompatibleConcept<typename C1::pixel_t, typename C2::pixel_t>>();
...
public:
    ...
    value_core(const type& val) : _pixel(val)
...
class reference_core
    : public value_core<typename std::remove_reference<PixelRef>::type::value_type, Tag>
{
public:
    using type = PixelRef;
    ...
    reference_core(const typename pixel_t::value_type& val)
        : parent_t(val)
        , _pixel(parent_t::_pixel)
    {
...
{
    using rgb565_pixel_t
        = packed_pixel_type<uint16_t, mp11::mp_list_c<unsigned, 5, 6, 5>, rgb_layout_t>::type;
    ...
    using bgr556_pixel_t
        = packed_pixel_type<uint16_t, mp11::mp_list_c<unsigned, 5, 6, 5>, bgr_layout_t>::type;
    ...
    get_color(r565, red_t())
        = channel_convert<kth_element_type<rgb565_pixel_t, 0>::type>(get_color(rgb_full, red_t()));
    get_color(r565, green_t()) = channel_convert<kth_element_type<rgb565_pixel_t, 1>::type>(
        get_color(rgb_full, green_t()));
    get_color(r565, blue_t())
        = channel_convert<kth_element_type<rgb565_pixel_t, 2>::type>(get_color(rgb_full, blue_t()));
    ...
    do_basic_test<value_core<rgb565_pixel_t, 0>, value_core<bgr556_pixel_t, 1>>(r565)
        .test_heterogeneous();
    ...
    using bgr121_ref_t = const bit_aligned_pixel_reference<
        std::uint8_t,
        mp11::mp_list_c<int, 1, 2, 1>,
        bgr_layout_t,
        true>;
    using rgb121_ref_t = const bit_aligned_pixel_reference<
        std::uint8_t,
        mp11::mp_list_c<int, 1, 2, 1>,
        rgb_layout_t,
        true>;
    ...
    rgb121_pixel_t p121;
    do_basic_test<reference_core<bgr121_ref_t, 0>, reference_core<rgb121_ref_t, 1>>(p121)
        .test_heterogeneous();
    do_basic_test<value_core<rgb121_pixel_t, 0>, reference_core<rgb121_ref_t, 1>>(p121)
        .test_heterogeneous();
...
void test_pixel()
{
    ...
    do_basic_test<value_core<bgr8_pixel_t, 0>, reference_core<rgb8_planar_ref_t, 1>>(rgb8)
        .test_all();
    do_basic_test<reference_core<rgb8_planar_ref_t, 0>, reference_core<bgr8_pixel_t&, 1>>(rgb8)
        .test_all();
    do_basic_test<reference_core<const rgb8_pixel_t&, 0>, reference_core<rgb8_pixel_t&, 1>>(rgb8)
        .test_all();
