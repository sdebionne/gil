...
#define RGB_VIEW(T)                                                                                \
    image_view<memory_based_2d_locator<memory_based_step_iterator<pixel<T, rgb_layout_t>*>>>
#define BGR_VIEW(T)                                                                                \
    image_view<memory_based_2d_locator<memory_based_step_iterator<pixel<T, bgr_layout_t>*>>>
#define RGB_PLANAR_VIEW(T)                                                                         \
    image_view<                                                                                    \
        memory_based_2d_locator<memory_based_step_iterator<planar_pixel_iterator<T*, rgb_t>>>>
...
void test_fill(std::size_t trials)
{
    ...
    std::cout << "Non-GIL: " << measure_time(fill_nongil_t<View, P>(view(im), P()), trials)
              << std::endl;
};
...
struct rgb_fr_t
{
    void operator()(pixel<T, rgb_layout_t>& p) const
    {
        p[0] = 0;
        p[1] = 1;
        p[2] = 2;
    }
    void operator()(const planar_pixel_reference<T&, rgb_t>& p) const
    {
        p[0] = 0;
        p[1] = 1;
        p[2] = 2;
...
struct for_each_gil_t
{
    ...
    void operator()() const
    {
        for_each_pixel(_v, _f);
    }
};
template <typename View, typename F>
struct for_each_nongil_t;
...
{
    image<typename View::value_type, is_planar<View>::value> im(width, height);
    std::cout << "GIL: " << measure_time(for_each_gil_t<View, F>(view(im), F()), trials)
              << std::endl;
    std::cout << "Non-GIL: " << measure_time(for_each_nongil_t<View, F>(view(im), F()), trials)
              << std::endl;
}
...
void test_copy(std::size_t trials)
{
    ...
    std::cout << "GIL: " << measure_time(copy_gil_t<View1, View2>(view(im1), view(im2)), trials)
              << std::endl;
    std::cout << "Non-GIL: "
              << measure_time(copy_nongil_t<View1, View2>(view(im1), view(im2)), trials)
              << std::endl;
}
...
struct bgr_to_rgb_t
{
    ...
    {
        return pixel<T, rgb_layout_t>(
            T(get_color(p, blue_t()) * 0.1f),
...
struct transform_gil_t
{
    ...
    View2 _v2;
    F _f;
    transform_gil_t(const View1& v1_in, const View2& v2_in, const F& f_in)
        : _v1(v1_in)
        , _v2(v2_in)
        , _f(f_in)
    {
    ...
    void operator()() const
    {
        transform_pixels(_v1, _v2, _f);
    }
};
template <typename View1, typename View2, typename F>
struct transform_nongil_t;
...
void test_performance()
{
    ...
    std::cout << "test copy_pixels() between rgb8_planar_image_t and rgb8_planar_image_t"
              << std::endl;
    ...
    test_transform<rgb8_view_t, rgb8_view_t, bgr_to_rgb_t<uint8_t, pixel<uint8_t, rgb_layout_t>>>(
        num_trials);
    std::cout << std::endl;

    std::cout << "test transform_pixels() between rgb8_planar_image_t and rgb8_planar_image_t"
              << std::endl;
    test_transform<
        rgb8_planar_view_t,
        rgb8_planar_view_t,
        bgr_to_rgb_t<uint8_t, planar_pixel_reference<uint8_t, rgb_t>>>(num_trials);
    std::cout << std::endl;

    std::cout << "test transform_pixels() between rgb8_image_t and rgb8_planar_image_t"
              << std::endl;
    test_transform<
        rgb8_view_t,
        rgb8_planar_view_t,
        bgr_to_rgb_t<uint8_t, pixel<uint8_t, rgb_layout_t>>>(num_trials);
    std::cout << std::endl;

    std::cout << "test transform_pixels() between rgb8_planar_image_t and rgb8_image_t"
              << std::endl;
    test_transform<
        rgb8_planar_view_t,
        rgb8_view_t,
