...
#ifdef _MSC_VER
#    pragma warning(                                                                               \
        disable : 4244)  // conversion from 'gil::image<V,Alloc>::coord_t' to 'int', possible loss of data (visual studio compiler doesn't realize that the two types are the same)
#    pragma warning(disable : 4503)  // decorated name length exceeded, name was truncated
#    pragma warning(                                                                               \
        disable : 4701)  // potentially uninitialized local variable 'result' used in boost/crc.hpp
#endif
...
struct mandelbrot_fn
{
    ...
    result_type operator()(const point_t& p) const
    {
        ...
        double t = get_num_iter(point<double>(
            p.x / (double)_img_size.x * 3 - 2, p.y / (double)_img_size.y * 3 - 1.0f));  //1.5f));
        t = pow(t, 0.2);

        value_type ret;
        for (std::size_t k = 0; k < num_channels<P>::value; ++k)
            ret[k] = (typename channel_type<
                      value_type>::type)(_in_color[k] * t + _out_color[k] * (1 - t));
        return ret;
    }
...
void image_test::basic_test(const string& prefix)
{
    ...
    Img imgWhite(img);
    for (typename View::iterator it = view(imgWhite).end(); (it - 1) != view(imgWhite).begin();
         --it)
    {
        if (*(it - 1) == red)
            *(it - 1) = white;
...
void image_test::view_transformations_test(const View& img_view, const string& prefix)
{
    ...
    check_view(
        subimage_view(
            img_view, iround(img_view.dimensions() / 4), iround(img_view.dimensions() / 2)),
        prefix + "cropped");
    check_view(color_converted_view<gray8_pixel_t>(img_view), prefix + "gray8");
    check_view(
        color_converted_view<gray8_pixel_t>(img_view, my_color_converter()), prefix + "my_gray8");
...
template <typename View>
void image_test::homogeneous_view_transformations_test(
    const View& img_view,
    const string& prefix,
    std::true_type)
{
...
{
    using any_image_t
        = any_image<gray8_image_t, bgr8_image_t, argb8_image_t, rgb8_image_t, rgb8_planar_image_t>;
    ...
    check_view(
        subsampled_view(rotated180_view(view(any_img)), 2, 1), "dynamic_subimage_subsampled180rot");
}
...
void checksum_image_test::check_view_impl(const rgb8c_view_t& img_view, const string& name)
{
    ...
    {
        cerr << "Checksum error in " << name << " (crc=" << std::hex << crc << " != " << std::hex
             << crc_expect << ")" << endl;
        error_if(true);
    }
}
...
const string ref_dir
    = in_dir + "image-ref/";  // reference directory to compare written with actual output
...
void static_checks()
{
    ...
    static_assert(
        std::is_same<derived_view_type<cmyk8c_planar_step_view_t>::type, cmyk8c_planar_step_view_t>::
            value,
        "");
    static_assert(
        std::is_same<
            derived_view_type<cmyk8c_planar_step_view_t, std::uint16_t, rgb_layout_t>::type,
