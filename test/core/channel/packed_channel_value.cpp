...
{
    static_assert(
        std::is_same<typename T::value_type, T>::value,
        "value_type should be the same as packed_channel_value specialization");

    static_assert(
        std::is_lvalue_reference<typename T::reference>::value,
    ...
    static_assert(std::is_pointer<typename T::pointer>::value, "pointer should be pointer type");

    static_assert(
        std::is_pointer<typename T::const_pointer>::value, "const_pointer should be pointer type");

    static_assert(T::is_mutable, "packed_channel_value should be mutable by default");

    static_assert(
        std::is_constructible<T, typename T::integer_t>::value,
        "packed_channel_value should be constructible from underlying integer_t");

    static_assert(
        std::is_convertible<T, typename T::integer_t>::value,
...
void test_packed_channel_value_with_num_bits_1()
{
    ...
    static_assert(
        std::is_same<bits1::integer_t, std::uint8_t>::value,
...
void test_packed_channel_value_with_num_bits_8()
{
    ...
    static_assert(
        std::is_same<bits8::integer_t, std::uint8_t>::value,
...
void test_packed_channel_value_with_num_bits15()
{
    ...
    static_assert(
        std::is_same<bits15::integer_t, std::uint16_t>::value,
