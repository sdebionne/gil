...
template <
    typename ChannelPtr,  // Models Channel Iterator (examples: unsigned char* or const unsigned char*)
...
public:
    ...
    interleaved_ptr(const interleaved_ptr<CP, Layout>& ptr) : _channels(ptr._channels)
    {
    }
    ...
    interleaved_ptr& operator=(reference* pix)
    {
        _channels = &((*pix)[0]);
        return *this;
    }
    ...
    const ChannelPtr& channels() const
    {
        return _channels;
    }
    ChannelPtr& channels()
...
private:
    ...
    template <typename CP, typename L>
    friend struct interleaved_ptr;
    ...
    {
        return _channels == it._channels;
    }

    reference dereference() const
    {
        return reference(_channels);
    }
};
...
inline std::ptrdiff_t memunit_distance(
    const interleaved_ptr<ChannelPtr, Layout>& p1,
    const interleaved_ptr<ChannelPtr, Layout>& p2)
{
...
template <typename ChannelPtr, typename Layout>
inline interleaved_ptr<ChannelPtr, Layout> memunit_advanced(
    const interleaved_ptr<ChannelPtr, Layout>& p,
    std::ptrdiff_t diff)
{
...
template <typename ChannelPtr, typename Layout>
inline typename interleaved_ptr<ChannelPtr, Layout>::reference memunit_advanced_ref(
    const interleaved_ptr<ChannelPtr, Layout>& p,
    std::ptrdiff_t diff)
{
    interleaved_ptr<ChannelPtr, Layout> ret = p;
