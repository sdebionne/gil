...
int main()
{
    ...
    write_view(
        "out-packed_pixel_bgr772.jpg",
        color_converted_view<bgr8_pixel_t>(transposed_view(const_view(bgr772_img))),
        jpeg_tag{});
    ...
    // Save the result. JPEG I/O does not support the packed pixel format, so convert it back to 8-bit RGB
    write_view(
        "out-packed_pixel_gray1.jpg",
        color_converted_view<gray8_pixel_t>(transposed_view(const_view(gray1_img))),
        jpeg_tag{});
