    ...
    threshold_direction direction = threshold_direction::regular)
{
    ...
    if (mode == threshold_truncate_mode::threshold)
    {
        ...
        {
            detail::threshold_impl<source_channel_t, result_channel_t>(
                src_view, dst_view, [threshold_value](source_channel_t px) -> result_channel_t {
...
void otsu_impl(SrcView const& src_view, DstView const& dst_view, threshold_direction direction)
{
    ...
    for (int t = 0; t < 256; t++)
    {
        ...
        var_intra_class
            = weight_back * weight_fore * (mean_back - mean_fore) * (mean_back - mean_fore);
    ...
    typename channel_type<DstView>::type constant = 0)
{
    ...
    if (direction == threshold_direction::regular)
    {
        detail::adaptive_impl<source_channel_t, result_channel_t>(
            src_view,
            temp_conv,
            dst_view,
            [max_value, constant](source_channel_t px, source_channel_t threshold)
                -> result_channel_t { return px > (threshold - constant) ? max_value : 0; });
    }
    else
    {
        detail::adaptive_impl<source_channel_t, result_channel_t>(
            ...
            dst_view,
            [max_value, constant](source_channel_t px, source_channel_t threshold)
                -> result_channel_t { return px > (threshold - constant) ? 0 : max_value; });
    }
}
