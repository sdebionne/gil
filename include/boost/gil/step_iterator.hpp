...
template <
    typename Derived,   // type of the derived class
...
{
public:
    using parent_t = iterator_adaptor<
        Derived,
        Iterator,
        use_default,
        use_default,
...
template <typename Iterator>
class memory_based_step_iterator
    : public detail::step_iterator_adaptor<
          ...
          memunit_step_fn<Iterator>>
{
    BOOST_GIL_CLASS_REQUIRE(Iterator, boost::gil, MemoryBasedIteratorConcept)
public:
    using parent_t = detail::step_iterator_adaptor<
        memory_based_step_iterator<Iterator>,
        Iterator,
        memunit_step_fn<Iterator>>;
    ...
    memory_based_step_iterator(Iterator it, std::ptrdiff_t memunit_step)
        : parent_t(it, memunit_step_fn<Iterator>(memunit_step))
    {
    }
...
template <typename Iterator>
inline std::ptrdiff_t memunit_step(const memory_based_step_iterator<Iterator>& p)
{
    return p.step();
}
...
template <typename Iterator>
inline void memunit_advance(memory_based_step_iterator<Iterator>& p, std::ptrdiff_t diff)
{
    memunit_advance(p.base(), diff);
}

template <typename Iterator>
inline memory_based_step_iterator<Iterator> memunit_advanced(
...
template <typename Iterator>
inline typename std::iterator_traits<Iterator>::reference memunit_advanced_ref(
    const memory_based_step_iterator<Iterator>& p,
    std::ptrdiff_t diff)
{
...
auto make_step_iterator_impl(
    ...
    std::ptrdiff_t step,
    std::true_type) -> memory_based_step_iterator<BaseIt>
{
