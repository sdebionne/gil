...
inline void bmp_read_view(const String& filename, const View& view)
{
    read_view(filename, view, bmp_tag());
}
...
inline void bmp_read_image(const String& filename, Image& img)
{
...
inline void bmp_read_and_convert_view(const String& filename, const View& view, CC cc)
{
    read_and_convert_view(filename, view, cc, bmp_tag());
}
...
/// Throws std::ios_base::failure if the file is not a valid BMP file, or if its dimensions don't match the ones of the view.
template <typename String, typename View>
inline void bmp_read_and_convert_view(const String& filename, const View& view)
{
    read_and_convert_view(filename, view, bmp_tag());
}
...
inline void bmp_read_and_convert_image(const String& filename, Image& img, CC cc)
{
    read_and_convert_image(filename, img, cc, bmp_tag());
}
...
/// Throws std::ios_base::failure if the file is not a valid BMP file
template <typename String, typename Image>
inline void bmp_read_and_convert_image(const String filename, Image& img)
{
    read_and_convert_image(filename, img, bmp_tag());
}
...
template <typename String, typename View>
inline void bmp_write_view(const String& filename, const View& view)
{
    write_view(filename, view, bmp_tag());
}
