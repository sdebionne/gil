...
class reader<Device, bmp_tag, ConversionPolicy>
...
public:
    ...
    // Constructor
    //
    reader(const Device& io_dev, const image_read_settings<bmp_tag>& settings)
    ...
    reader(
        ...
        : reader_base<bmp_tag, ConversionPolicy>(cc)
        , backend_t(io_dev, settings)
        , _pitch(0)
    {
    }
    ...
    void apply(const View& dst_view)
    {
        ...
        io_error_if(
            !detail::is_allowed<View>(this->_info, is_read_and_convert_t()),
        ...
        {
            _pitch
                = static_cast<long>(((this->_info._width * this->_info._bits_per_pixel) + 7) >> 3);
        }
        else
        {
            _pitch
        ...
        case 1:
        {
            this->_scanline_length = (this->_info._width * num_channels<rgba8_view_t>::value + 3)
                                   & ~3;

            read_palette_image<
                gray1_image_t::view_t,
                detail::mirror_bits<byte_vector_t, std::true_type>>(dst_view);
            break;
        }
        ...
        case 4:
        {
            ...
            case bmp_compression::_rgb:
            {
                ...
                read_palette_image<
                    ...
                    detail::swap_half_bytes<byte_vector_t, std::true_type>>(dst_view);
                break;
            }
        ...
        case 8:
        {
            ...
            case bmp_compression::_rgb:
            {
                ...
                read_palette_image<
                    ...
                    detail::do_nothing<std::vector<gray8_pixel_t>>>(dst_view);
                break;
            }
        ...
        case 16:
        {
            this->_scanline_length = (this->_info._width * num_channels<rgb8_view_t>::value + 3)
                                   & ~3;
...
private:
    long get_offset(std::ptrdiff_t pos)
    {
        if (this->_info._height > 0)
        {
            ...
            return static_cast<long>(
                (this->_info._offset + (this->_info._height - 1 - pos) * _pitch));
        }
        else
        {
            return static_cast<long>((this->_info._offset + pos * _pitch));
        }
    }

    template <typename View_Src, typename Byte_Manipulator, typename View_Dst>
    void read_palette_image(const View_Dst& view)
    {
        ...
        for (std::ptrdiff_t y = 0; y < this->_settings._dim.y; ++y)
        {
            ...
            this->_io_dev.read(reinterpret_cast<byte_t*>(rh.data()), _pitch);
    ...
    void read_data_15(const View& view)
    {
        ...
        else if (this->_info._compression == bmp_compression::_rgb)
        {
            ...
            case 16:
            {
                this->_mask.red.mask = 0x007C00;
                this->_mask.red.width = 5;
                this->_mask.red.shift = 10;
                this->_mask.green.mask = 0x0003E0;
                this->_mask.green.width = 5;
                this->_mask.green.shift = 5;
                this->_mask.blue.mask = 0x00001F;
                this->_mask.blue.width = 5;
                this->_mask.blue.shift = 0;
            ...
            case 32:
            {
                this->_mask.red.mask = 0xFF0000;
                this->_mask.red.width = 8;
                this->_mask.red.shift = 16;
                this->_mask.green.mask = 0x00FF00;
                this->_mask.green.width = 8;
                this->_mask.green.shift = 8;
                this->_mask.blue.mask = 0x0000FF;
                this->_mask.blue.width = 8;
                this->_mask.blue.shift = 0;
        ...
        for (std::ptrdiff_t y = 0; y < this->_settings._dim.y; ++y)
        {
            ...
            for (int32_t i = 0; i < this->_info._width; ++i, src += 2)
            {
                ...
                int r = ((p & this->_mask.red.mask) >> this->_mask.red.shift)
                     << (8 - this->_mask.red.width);
                int g = ((p & this->_mask.green.mask) >> this->_mask.green.shift)
                     << (8 - this->_mask.green.width);
                int b = ((p & this->_mask.blue.mask) >> this->_mask.blue.shift)
                     << (8 - this->_mask.blue.width);
    ...
    void copy_row_if_needed(const Buffer& buf, const View& view, std::ptrdiff_t y)
    {
        if (y >= this->_settings._top_left.y && y < this->_settings._dim.y)
        {
            ...
            std::copy(beg, end, view.row_begin(y));
        }
    }
...
{
public:
    bmp_type_format_checker(const bmp_bits_per_pixel::type& bpp) : _bpp(bpp)
    {
    }
    ...
    bool apply()
    {
        if (_bpp < 32)
        {
            return pixels_are_compatible<typename Image::value_type, rgb8_pixel_t>::value ? true
                                                                                          : false;
        }
        else
        {
            return pixels_are_compatible<typename Image::value_type, rgba8_pixel_t>::value ? true
                                                                                           : false;
        }
    }
...
private:
    // to avoid C4512
    bmp_type_format_checker& operator=(const bmp_type_format_checker&)
    {
        return *this;
    }
...
{
    template <typename View>
    struct apply : public is_read_supported<typename get_pixel_type<View>::type, bmp_tag>
    {
    };
};
...
public:
    dynamic_image_reader(const Device& io_dev, const image_read_settings<bmp_tag>& settings)
    ...
    void apply(any_image<Images...>& images)
    {
        ...
        else
        {
            ...
            detail::dynamic_io_fnobj<detail::bmp_read_is_supported, parent_t> op(this);
