...
class scanline_reader<Device, bmp_tag> : public reader_backend<Device, bmp_tag>
{
public:
    using tag_t = bmp_tag;
...
public:
    ...
    // Constructor
    //
    scanline_reader(Device& device, const image_read_settings<bmp_tag>& settings)
    ...
    void read(byte_t* dst, int pos)
    {
        ...
        if (this->_info._height > 0)
        {
            ...
            offset = this->_info._offset + (this->_info._height - 1 - pos) * this->_pitch;
        }
        else
        {
            offset = this->_info._offset + pos * _pitch;
        }
...
private:
    void initialize()
    {
        ...
        case 4:
        {
            ...
            {
                io_error(
                    "Cannot read run-length encoded images in iterator mode. Try to read as whole "
                    "image.");
    ...
    void read_bit_row(byte_t* dst)
    {
        ...
        src_view_t src_view = interleaved_view(
            this->_info._width,
            1,
    ...
    void read_15_bits_row(byte_t* dst)
    {
        ...
        dst_view_t dst_view = interleaved_view(
            this->_info._width, 1, (typename dst_view_t::value_type*)dst, this->_pitch);
        ...
        for (dst_view_t::x_coord_t i = 0; i < this->_info._width; ++i, src += 2)
        {
            int p = (src[1] << 8) | src[0];
