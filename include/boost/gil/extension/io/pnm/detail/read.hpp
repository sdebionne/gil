...
class reader<Device, pnm_tag, ConversionPolicy>
    : public reader_base<pnm_tag, ConversionPolicy>
...
public:
    ...
    reader(const Device& io_dev, const image_read_settings<pnm_tag>& settings)
        : reader_base<pnm_tag, ConversionPolicy>()
        , backend_t(io_dev, settings)
    {
    }

    reader(const Device& io_dev, const cc_t& cc, const image_read_settings<pnm_tag>& settings)
        : reader_base<pnm_tag, ConversionPolicy>(cc)
        , backend_t(io_dev, settings)
    {
    }
...
private:
        ...
        bool process)
    {
        ...
        for (uint32_t x = 0; x < this->_scanline_length; ++x)
        {
            ...
            if (process)
            {
                ...
                {
                    using channel_t =
                        typename channel_type<typename get_pixel_type<View_Dst>::type>::type;

                    // for pnm format 0 is white
                    row[x] = (value != 0) ? typename channel_traits<channel_t>::value_type(0)
        ...
        if (process)
        {
            ...
            copy_data<View_Dst, View_Src>(
                dst, src, y, typename std::is_same<View_Dst, gray1_image_t::view_t>::type());
        }
    }
    ...
    )
    {
        if (this->_info._max_value == 1)
        {
            ...
            for (typename View_Dst::x_coord_t x = 0; x < dst.width(); ++x)
            {
                it[x] = src[x];
            }
        }
    ...
    void read_bin_data(const View_Dst& view)
    {
        ...
        {
            this->_io_dev.read(reinterpret_cast<byte_t*>(rh.data()), this->_scanline_length);
        }
...
{
    pnm_type_format_checker(pnm_image_type::type type) : _type(type)
    {
    }
    ...
    bool apply()
    {
        using is_supported_t
            = is_read_supported<typename get_pixel_type<typename Image::view_t>::type, pnm_tag>;

        return is_supported_t::_asc_type == _type || is_supported_t::_bin_type == _type;
    }
...
{
    template <typename View>
    struct apply : public is_read_supported<typename get_pixel_type<View>::type, pnm_tag>
    {
    };
};
...
class dynamic_image_reader<Device, pnm_tag>
    : public reader<Device, pnm_tag, detail::read_and_no_convert>
{
    using parent_t = reader<Device, pnm_tag, detail::read_and_no_convert>;

public:
    dynamic_image_reader(const Device& io_dev, const image_read_settings<pnm_tag>& settings)
    ...
    void apply(any_image<Images...>& images)
    {
        ...
        else
        {
            ...
            detail::dynamic_io_fnobj<detail::pnm_read_is_supported, parent_t> op(this);
