...
struct pnm_write_is_supported
{
    ...
    struct apply : public is_write_supported<typename get_pixel_type<View>::type, pnm_tag>
    {
    };
};
...
class writer<Device, pnm_tag> : public writer_backend<Device, pnm_tag>
{
private:
    using backend_t = writer_backend<Device, pnm_tag>;

public:
    writer(const Device& io_dev, const image_write_info<pnm_tag>& info) : backend_t(io_dev, info)
    {
    }
    ...
    void apply(const View& view)
    {
        ...
        // write data
        write_data(view, pitch, typename is_bit_aligned<pixel_t>::type());
    }
...
private:
    ...
    {
        return mp11::
            mp_if_c<Channels == 1, pnm_image_type::mono_bin_t, pnm_image_type::color_bin_t>::value;
    }
    ...
    {
        return mp11::
            mp_if_c<Channels == 1, pnm_image_type::gray_bin_t, pnm_image_type::color_bin_t>::value;
    }
    ...
    )
    {
        ...
        for (typename View::y_coord_t y = 0; y < src.height(); ++y)
        {
            ...
            std::copy(src.row_begin(y), src.row_end(y), buf.begin());
...
///
template <typename Device>
class dynamic_image_writer<Device, pnm_tag> : public writer<Device, pnm_tag>
{
    using parent_t = writer<Device, pnm_tag>;

public:
    dynamic_image_writer(const Device& io_dev, const image_write_info<pnm_tag>& info)
    ...
    {
        detail::dynamic_io_fnobj<detail::pnm_write_is_supported, parent_t> op(this);
