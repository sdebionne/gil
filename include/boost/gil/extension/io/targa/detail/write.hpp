...
template <int N>
struct get_targa_view_type
{
};
template <>
struct get_targa_view_type<3>
{
    using type = bgr8_view_t;
};
template <>
struct get_targa_view_type<4>
{
...
struct targa_write_is_supported
{
    ...
    struct apply : public is_write_supported<typename get_pixel_type<View>::type, targa_tag>
    {
    };
};
...
public:
    writer(const Device& io_dev, const image_write_info<targa_tag>& info) : backend_t(io_dev, info)
    {
    }
...
private:
    ...
    void write(const View& view)
    {
        ...
        write_image<View, typename detail::get_targa_view_type<num_channels<View>::value>::type>(
            view);
    }


    template <typename View, typename TGA_View>
    void write_image(const View& view)
    {
        ...
        {
            copy_pixels(
                subimage_view(view, 0, static_cast<int>(y), static_cast<int>(view.width()), 1),
                row);
...
///
template <typename Device>
class dynamic_image_writer<Device, targa_tag> : public writer<Device, targa_tag>
{
    using parent_t = writer<Device, targa_tag>;

public:
    dynamic_image_writer(const Device& io_dev, const image_write_info<targa_tag>& info)
    ...
    {
        detail::dynamic_io_fnobj<detail::targa_write_is_supported, parent_t> op(this);
