...
class reader<Device, targa_tag, ConversionPolicy>
...
public:
    ...
    reader(const Device& io_dev, const cc_t& cc, const image_read_settings<targa_tag>& settings)
        : reader_base<targa_tag, ConversionPolicy>(cc)
        , backend_t(io_dev, settings)
    {
    }
...
{
public:
    targa_type_format_checker(const targa_depth::type& bpp) : _bpp(bpp)
    {
    }
...
private:
    // to avoid C4512
    targa_type_format_checker& operator=(const targa_type_format_checker&)
    {
        return *this;
    }
...
{
    template <typename View>
    struct apply : public is_read_supported<typename get_pixel_type<View>::type, targa_tag>
    {
    };
};
...
public:
    dynamic_image_reader(const Device& io_dev, const image_read_settings<targa_tag>& settings)
    ...
    void apply(any_image<Images...>& images)
    {
        ...
        else
        {
            ...
            detail::dynamic_io_fnobj<detail::targa_read_is_supported, parent_t> op(this);
