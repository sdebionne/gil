...
struct writer_backend<Device, png_tag> : public detail::png_struct_info_wrapper
{
private:
...
public:
    ...
    /// Constructor
    ///
    writer_backend(const Device& io_dev, const image_write_info<png_tag>& info)
        ...
        , _info(info)
    {
        ...
        get()->_struct = png_create_write_struct(
        ...
        io_error_if(get_struct() == nullptr, "png_writer: fail to call png_create_write_struct()");
        ...
        if (get_info() == nullptr)
        {
            png_destroy_write_struct(&get()->_struct, nullptr);
        ...
        if (setjmp(png_jmpbuf(get_struct())))
        {
            ...
            png_destroy_write_struct(&get()->_struct, &get()->_info);
...
protected:
    ...
    void write_header(const View& view)
    {
        ...
        {
            png_set_gAMA(get_struct(), get_info(), _info._file_gamma);
        }
#else
        if (_info._valid_cie_colors)
        {
        ...
        if (_info._valid_file_gamma)
        {
            png_set_gAMA_fixed(get_struct(), get_info(), _info._file_gamma);
        }
        ...
        {
            png_set_sRGB(get_struct(), get_info(), _info._intent);
        }
        ...
        {
            png_set_bKGD(get_struct(), get_info(), const_cast<png_color_16p>(&_info._background));
        }

        if (_info._valid_histogram)
        {
            png_set_hIST(
                get_struct(), get_info(), const_cast<png_uint_16p>(&_info._histogram.front()));
        }

        if (_info._valid_offset)
        {
            png_set_oFFs(
                get_struct(), get_info(), _info._offset_x, _info._offset_y, _info._off_unit_type);
        }
        ...
        {
            png_set_pHYs(
                get_struct(), get_info(), _info._res_x, _info._res_y, _info._phy_unit_type);
        }

        if (_info._valid_significant_bits)
        {
            png_set_sBIT(get_struct(), get_info(), const_cast<png_color_8p>(&_info._sig_bits));
        }
...
#    endif      // BOOST_GIL_IO_PNG_FLOATING_POINT_SUPPORTED
        ...
        if (_info._valid_text)
        {
            ...
            png_set_text(get_struct(), get_info(), &texts.front(), _info._num_text);
        }

        if (_info._valid_modification_time)
        {
            png_set_tIME(get_struct(), get_info(), const_cast<png_timep>(&_info._mod_time));
        }
...
*/
        ...
        png_set_compression_level(get_struct(), _info._compression_level);

        png_set_compression_mem_level(get_struct(), _info._compression_mem_level);

        png_set_compression_strategy(get_struct(), _info._compression_strategy);

        png_set_compression_window_bits(get_struct(), _info._compression_window_bits);

        png_set_compression_method(get_struct(), _info._compression_method);

        png_set_compression_buffer_size(get_struct(), _info._compression_buffer_size);
        ...
        {
            png_set_filter(get_struct(), 0, _info._filter);
        }
        ...
        {
            png_set_sBIT(get_struct(), get_info(), &_info._true_bits.front());
        }

        // sRGB Intent
        if (_info._set_srgb_intent)
        {
            png_set_sRGB(get_struct(), get_info(), _info._srgb_intent);
        }
        ...
        png_write_info(get_struct(), get_info());
    }
