...
class reader<Device, png_tag, ConversionPolicy>
...
public:
    reader(const Device& io_dev, const image_read_settings<png_tag>& settings)
        : reader_base<png_tag, ConversionPolicy>()
    ...
    reader(
        ...
        : reader_base<png_tag, ConversionPolicy>(cc)
        , backend_t(io_dev, settings)
    {
    }
    ...
    void apply(const View& view)
    {
        ...
        if (this->_settings._apply_screen_gamma)
        {
            ...
            png_set_gamma(
                this->get_struct(), this->_settings._screen_gamma, this->_info._file_gamma);
#else
            png_set_gamma(
                this->get_struct(), this->_settings._screen_gamma, this->_info._file_gamma);
        ...
        png_read_update_info(this->get_struct(), this->get_info());

        this->_info._bit_depth = png_get_bit_depth(this->get_struct(), this->get_info());

        this->_info._num_channels = png_get_channels(this->get_struct(), this->get_info());

        this->_info._color_type = png_get_color_type(this->get_struct(), this->get_info());

        this->_scanline_length = png_get_rowbytes(this->get_struct(), this->get_info());
        ...
        case PNG_COLOR_TYPE_GRAY:
        {
            ...
            {
            case 1:
                read_rows<gray1_image_t::view_t::reference>(view);
                break;
            case 2:
                read_rows<gray2_image_t::view_t::reference>(view);
                break;
            case 4:
                read_rows<gray4_image_t::view_t::reference>(view);
                break;
            case 8:
            ...
            default:
                io_error(
                    "png_reader::read_data(): unknown combination of color type and bit depth");
            }
...
#else
            io_error(
                "gray_alpha isn't enabled. Define BOOST_GIL_IO_ENABLE_GRAY_ALPHA when building "
                "application.");
        ...
        case PNG_COLOR_TYPE_RGBA:
        {
            ...
            default:
                io_error(
                    "png_reader_color_convert::read_data(): unknown combination of color type and "
                    "bit depth");
            }
...
private:
    ...
    void read_rows(const View& view)
    {
        ...
        std::size_t rowbytes = png_get_rowbytes(this->get_struct(), this->get_info());

        row_buffer_helper_t buffer(rowbytes, true);
        ...
        for (std::size_t pass = 0; pass < this->_number_passes; pass++)
        {
            if (pass == this->_number_passes - 1)
            {
                ...
                for (std::ptrdiff_t y = 0; y < this->_settings._dim.y; ++y)
                {
                    ...
                    this->_cc_policy.read(first, last, view.row_begin(y));
                }
...
{
    png_type_format_checker(png_bitdepth::type bit_depth, png_color_type::type color_type)
    ...
    bool apply()
    {
        using is_supported_t
            = is_read_supported<typename get_pixel_type<typename Image::view_t>::type, png_tag>;
...
{
    template <typename View>
    struct apply : public is_read_supported<typename get_pixel_type<View>::type, png_tag>
    {
    };
};
...
public:
    dynamic_image_reader(const Device& io_dev, const image_read_settings<png_tag>& settings)
    ...
    {
        detail::png_type_format_checker format_checker(
            this->_info._bit_depth, this->_info._color_type);
        ...
        else
        {
            ...
            detail::dynamic_io_fnobj<detail::png_read_is_supported, parent_t> op(this);
