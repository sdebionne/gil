...
struct png_write_is_supported
{
    ...
    struct apply : public is_write_supported<typename get_pixel_type<View>::type, png_tag>
    {
    };
};
...
public:
    ...
    writer(const Device& io_dev, const image_write_info<png_tag>& info) : backend_t(io_dev, info)
    {
    }
    ...
    {
        io_error_if(
            view.width() == 0 && view.height() == 0, "png format cannot handle empty views.");

        this->write_header(view);

        write_view(view, typename is_bit_aligned<typename View::value_type>::type());
    }
...
private:
    ...
    )
    {
        ...
        png_write_end(this->get_struct(), this->get_info());
    }
    ...
    )
    {
        ...
        detail::row_buffer_helper_view<View> row_buffer(view.width(), false);
        ...
        png_free_data(this->get_struct(), this->get_info(), PNG_FREE_UNKN, -1);
    ...
    struct is_less_than_eight
        : mp11::mp_less<std::integral_constant<int, Info::_bit_depth>, std::integral_constant<int, 8>>
    {
    };

    template <typename Info>
    struct is_equal_to_sixteen
        : mp11::mp_less<std::integral_constant<int, Info::_bit_depth>, std::integral_constant<int, 16>>
    {
    };
...
public:
    dynamic_image_writer(const Device& io_dev, const image_write_info<png_tag>& info)
    ...
    {
        detail::dynamic_io_fnobj<detail::png_write_is_supported, parent_t> op(this);
