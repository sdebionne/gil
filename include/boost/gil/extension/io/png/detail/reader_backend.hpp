...
#    pragma warning(                                                                               \
        disable : 4611)  //interaction between '_setjmp' and C++ object destruction is non-portable
#endif
...
struct reader_backend<Device, png_tag> : public detail::png_struct_info_wrapper
{
public:
...
public:
    reader_backend(const Device& io_dev, const image_read_settings<png_tag>& settings)
        : _io_dev(io_dev)
    ...
    void read_header()
    {
        ...
        io_error_if(
            _io_dev.read(buf, PNG_BYTES_TO_CHECK) != PNG_BYTES_TO_CHECK,
            "png_check_validity: failed to read image");

        io_error_if(
            png_sig_cmp(png_bytep(buf), png_size_t(0), PNG_BYTES_TO_CHECK) != 0,
        ...
        get()->_struct = png_create_read_struct(
        ...
        io_error_if(
            get()->_struct == nullptr, "png_reader: fail to call png_create_write_struct()");
        ...
        user_chunk_data[2] = 0;
        user_chunk_data[3] = 0;
        png_set_read_user_chunk_fn(get_struct(), user_chunk_data, this_t::read_user_chunk_callback);
        ...
        if (get_info() == nullptr)
        {
            png_destroy_read_struct(&get()->_struct, nullptr, nullptr);
        ...
        if (setjmp(png_jmpbuf(get_struct())))
        {
            ...
            png_destroy_read_struct(&get()->_struct, &get()->_info, nullptr);

            io_error("png is invalid");
        }

        png_set_read_fn(get_struct(), static_cast<png_voidp>(&this->_io_dev), this_t::read_data);

        // Set up a callback function that will be
        // called after each row has been read, which you can use to control
        // a progress meter or the like.
        png_set_read_status_fn(get_struct(), this_t::read_row_callback);

        // Set up a callback which implements user defined transformation.
        // @todo
        png_set_read_user_transform_fn(get_struct(), png_user_transform_ptr(nullptr));

        png_set_keep_unknown_chunks(get_struct(), PNG_HANDLE_CHUNK_ALWAYS, nullptr, 0);


        // Make sure we read the signature.
        // @todo make it an option
        png_set_sig_bytes(get_struct(), PNG_BYTES_TO_CHECK);

        // The call to png_read_info() gives us all of the information from the
        // PNG file before the first IDAT (image data chunk).  REQUIRED
        png_read_info(get_struct(), get_info());
        ...
        this->_info._num_channels = png_get_channels(get_struct(), get_info());
        ...
        {
            this->_info._valid_cie_colors = png_get_cHRM(
                get_struct(),
                get_info(),
                &this->_info._white_x,
                &this->_info._white_y,
        ...
        {
            this->_info._valid_file_gamma
                = png_get_gAMA(get_struct(), get_info(), &this->_info._file_gamma);
        ...
        if (this->_settings._read_cie_chromacities)
        {
            this->_info._valid_cie_colors = png_get_cHRM_fixed(
                get_struct(),
                get_info(),
                &this->_info._white_x,
                &this->_info._white_y,
        ...
        {
            this->_info._valid_file_gamma
                = png_get_gAMA_fixed(get_struct(), get_info(), &this->_info._file_gamma);
        ...
        if (this->_settings._read_icc_profile)
        {
            ...
            this->_info._valid_icc_profile = png_get_iCCP(
                get_struct(),
                get_info(),
                &icc_name,
            ...
            {
                std::copy_n(
                    profile, this->_info._profile_length, std::back_inserter(this->_info._profile));
            }
        }
        ...
        {
            this->_info._valid_intent
                = png_get_sRGB(get_struct(), get_info(), &this->_info._intent);
        }
        ...
        if (this->_settings._read_palette)
        {
            ...
            this->_info._valid_palette
                = png_get_PLTE(get_struct(), get_info(), &palette, &this->_info._num_palette);

            if (this->_info._num_palette > 0)
            {
                this->_info._palette.resize(this->_info._num_palette);
                std::copy(
                    palette, palette + this->_info._num_palette, &this->_info._palette.front());
            }
        }
        ...
        if (this->_settings._read_background)
        {
            ...
            this->_info._valid_background = png_get_bKGD(get_struct(), get_info(), &background);
            if (background)
            {
        ...
        if (this->_settings._read_histogram)
        {
            ...
            this->_info._valid_histogram = png_get_hIST(get_struct(), get_info(), &histogram);
        ...
        {
            this->_info._valid_offset = png_get_oFFs(
                get_struct(),
                get_info(),
        ...
        if (this->_settings._read_pixel_calibration)
        {
            ...
            this->_info._valid_pixel_calibration = png_get_pCAL(
                get_struct(),
                get_info(),
                &purpose,
                &this->_info._X0,
            ...
            if (purpose)
            {
                this->_info._purpose.append(purpose, std::strlen(purpose));
            }

            if (units)
            {
                this->_info._units.append(units, std::strlen(units));
            }
            ...
            if (this->_info._num_params > 0)
            {
                ...
                for (png_CAL_nparam::type i = 0; i < this->_info._num_params; ++i)
                {
                    this->_info._params[i].append(params[i], std::strlen(params[i]));
                }
            }
        }
        ...
        {
            this->_info._valid_resolution = png_get_pHYs(
                get_struct(),
                get_info(),
                &this->_info._res_x,
                &this->_info._res_y,
        ...
        {
            this->_info._pixels_per_meter = png_get_pixels_per_meter(get_struct(), get_info());
        }
        ...
        if (this->_settings._read_number_of_significant_bits)
        {
            ...
            this->_info._valid_significant_bits = png_get_sBIT(get_struct(), get_info(), &sig_bits);
...
#    ifdef BOOST_GIL_IO_PNG_FLOATING_POINT_SUPPORTED
        ...
        {
            this->_info._valid_scale_factors = png_get_sCAL(
                get_struct(),
                get_info(),
...
#        ifdef BOOST_GIL_IO_PNG_FIXED_POINT_SUPPORTED
        if (this->_settings._read_scale_factors)
        {
            this->_info._valid_scale_factors = png_get_sCAL_fixed(
                get_struct(),
                get_info(),
                &this->_info._scale_unit,
                &this->_info._scale_width,
...
#        else
        if (this->_settings._read_scale_factors)
        {
            ...
            if (this->_info._valid_scale_factors)
            {
                if (scale_width)
                {
                    this->_info._scale_width.append(scale_width, std::strlen(scale_width));
                }

                if (scale_height)
                {
                    this->_info._scale_height.append(scale_height, std::strlen(scale_height));
                }
            }
        }
...
#    endif      // BOOST_GIL_IO_PNG_FLOATING_POINT_SUPPORTED
        ...
        if (this->_settings._read_comments)
        {
            ...
            this->_info._valid_text
                = png_get_text(get_struct(), get_info(), &text, &this->_info._num_text);

            if (this->_info._num_text > 0)
            {
                this->_info._text.resize(this->_info._num_text);

                for (png_num_text::type i = 0; i < this->_info._num_text; ++i)
                {
                    this->_info._text[i]._compression = text[i].compression;
                    this->_info._text[i]._key.append(text[i].key, std::strlen(text[i].key));

                    this->_info._text[i]._text.append(text[i].text, std::strlen(text[i].text));
                }
            }
        }
        ...
        if (this->_settings._read_last_modification_time)
        {
            ...
            this->_info._valid_modification_time
                = png_get_tIME(get_struct(), get_info(), &mod_time);
            if (mod_time)
            {
        ...
        if (this->_settings._read_transparency_data)
        {
            ...
            this->_info._valid_transparency_factors = png_get_tRNS(
                get_struct(), get_info(), &trans, &this->_info._num_trans, &trans_values);
...
protected:
    ...
    static int read_user_chunk_callback(
        png_struct* /* png_ptr */
        ,
