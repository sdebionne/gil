...
inline point_t png_read_dimensions(String const& filename)
{
    ...
    return {
        static_cast<std::ptrdiff_t>(backend._info._width),
        static_cast<std::ptrdiff_t>(backend._info._height)};
}
...
inline void png_read_view(const String& filename, const View& view)
{
    read_view(filename, view, png_tag());
}
...
inline void png_read_image(const String& filename, Image& img)
{
...
inline void png_read_and_convert_view(const String& filename, const View& view, CC cc)
{
    read_and_convert_view(filename, view, cc, png_tag());
}
...
/// Throws std::ios_base::failure if the file is not a valid PNG file, or if its dimensions don't match the ones of the view.
template <typename String, typename View>
inline void png_read_and_convert_view(const String& filename, const View& view)
{
    read_and_convert_view(filename, view, png_tag());
}
...
inline void png_read_and_convert_image(const String& filename, Image& img, CC cc)
{
    read_and_convert_image(filename, img, cc, png_tag());
}
...
/// Throws std::ios_base::failure if the file is not a valid PNG file
template <typename String, typename Image>
inline void png_read_and_convert_image(const String filename, Image& img)
{
...
template <typename String, typename View>
inline void png_write_view(const String& filename, const View& view)
{
    write_view(filename, view, png_tag());
}
