...
{
    template <typename View, typename Device, typename ConversionPolicy>
    static void read_plane(const View& dst_view, reader<Device, tiff_tag, ConversionPolicy>* p)
    {
...
struct plane_recursion<-1>
{
    ...
    static void read_plane(
        const View& /* dst_view */
        ,
        reader<Device, tiff_tag, ConversionPolicy>* /* p         */
    )
    {
    }
};
...
class reader<Device, tiff_tag, ConversionPolicy>
    : public reader_base<tiff_tag, ConversionPolicy>

    , public reader_backend<Device, tiff_tag>
{
private:
...
public:
    ...
    reader(const Device& io_dev, const image_read_settings<tiff_tag>& settings)
        : reader_base<tiff_tag, ConversionPolicy>()
    ...
    reader(
        ...
        : reader_base<tiff_tag, ConversionPolicy>(cc)
        , backend_t(io_dev, settings)
    {
    }
    ...
    void apply(View& dst_view)
    {
        ...
        else
        {
            ...
            using is_read_only =
                typename std::is_same<ConversionPolicy, detail::read_and_no_convert>::type;

            io_error_if(
                !detail::is_allowed<View>(this->_info, is_read_only()),
                "Image types aren't compatible.");

            if (this->_info._planar_configuration == PLANARCONFIG_SEPARATE)
            {
                plane_recursion<num_channels<View>::value - 1>::read_plane(dst_view, this);
            }
            else if (this->_info._planar_configuration == PLANARCONFIG_CONTIG)
            {
                read(dst_view, typename is_read_only::type());
            }
            else
            {
...
private:
    ...
    )
    {
        ...
        case PHOTOMETRIC_MINISBLACK:
        {
            ...
            case 1:
            {
                read_data<detail::row_buffer_helper_view<gray1_image_t::view_t>>(v, 0);
                break;
            }
            case 2:
            {
                read_data<detail::row_buffer_helper_view<gray2_image_t::view_t>>(v, 0);
                break;
            }
            case 4:
            {
                read_data<detail::row_buffer_helper_view<gray4_image_t::view_t>>(v, 0);
                break;
            }
            case 8:
            {
                read_data<detail::row_buffer_helper_view<gray8_view_t>>(v, 0);
                break;
            }
            case 16:
            {
                read_data<detail::row_buffer_helper_view<gray16_view_t>>(v, 0);
                break;
            }
            case 32:
            {
                read_data<detail::row_buffer_helper_view<gray32_view_t>>(v, 0);
                break;
            }
            default:
            {
        ...
        case PHOTOMETRIC_RGB:
        {
            ...
            case 3:
            {
                ...
                case 8:
                {
                    read_data<detail::row_buffer_helper_view<rgb8_view_t>>(v, 0);
                    break;
                }
                case 16:
                {
                    read_data<detail::row_buffer_helper_view<rgb16_view_t>>(v, 0);
                    break;
                }
                case 32:
                {
                    read_data<detail::row_buffer_helper_view<rgb32_view_t>>(v, 0);
                    break;
                }
                default:
                {
            ...
            case 4:
            {
                ...
                case 8:
                {
                    read_data<detail::row_buffer_helper_view<rgba8_view_t>>(v, 0);
                    break;
                }
                case 16:
                {
                    read_data<detail::row_buffer_helper_view<rgba16_view_t>>(v, 0);
                    break;
                }
                case 32:
                {
                    read_data<detail::row_buffer_helper_view<rgba32_view_t>>(v, 0);
                    break;
                }
                default:
                {
        ...
        case PHOTOMETRIC_SEPARATED:  // CYMK
        {
            ...
            case 8:
            {
                read_data<detail::row_buffer_helper_view<cmyk8_view_t>>(v, 0);
                break;
            }
            case 16:
            {
                read_data<detail::row_buffer_helper_view<cmyk16_view_t>>(v, 0);
                break;
            }
            case 32:
            {
                read_data<detail::row_buffer_helper_view<cmyk32_view_t>>(v, 0);
                break;
            }
            default:
            {
    ...
    template <typename PaletteImage, typename View>
    void read_palette_image(const View& dst_view)
    {
        PaletteImage indices(
            this->_info._width - this->_settings._top_left.x,
        ...
        read_palette_image(
            dst_view, view(indices), typename std::is_same<View, rgb16_view_t>::type());
    }

    template <typename View, typename Indices_View>
    void read_palette_image(
        const View& dst_view,
    ...
    )
    {
        ...
        using channel_t = typename channel_traits<
            typename element_type<typename Indices_View::value_type>::type>::value_type;

        int num_colors = channel_traits<channel_t>::max_value();

        rgb16_planar_view_t palette
            = planar_rgb_view(num_colors, 1, red, green, blue, sizeof(uint16_t) * num_colors);
    ...
    template <typename View, typename Indices_View>
    inline void read_palette_image(
        const View& /* dst_view     */
        ,
        const Indices_View& /* indices_view */
        ,
    ...
    void skip_over_rows(Buffer& buffer, int plane)
    {
        if (this->_info._compression != COMPRESSION_NONE)
        {
            ...
            {
                this->_io_dev.read_scanline(buffer, row, static_cast<tsample_t>(plane));
            }
        }
    }

    template <typename Buffer, typename View>
    void read_data(const View& dst_view, int /* plane */)
    {
    ...
    template <typename Buffer, typename View>
    void read_tiled_data(const View& dst_view, int plane)
    {
        if (dst_view.width() != this->_info._width || dst_view.height() != this->_info._height)
        {
            // read a subimage
    ...
    template <typename Buffer, typename View>
    void read_tiled_data_subimage(const View& dst_view, int plane)
    {
        ///@todo: why is
        ...
        for (unsigned int y = 0; y < image_height; y += tile_height)
        {
            ...
            {
                uint32_t current_tile_width = (x + tile_width < image_width) ? tile_width
                                                                             : image_width - x;
                uint32_t current_tile_length = (y + tile_height < image_height) ? tile_height
                                                                                : image_height - y;

                this->_io_dev.read_tile(
                    row_buffer_helper.buffer(), x, y, 0, static_cast<tsample_t>(plane));
                ...
                point_t view_lower_right(
                    subimage_x + subimage_width - 1, subimage_y + subimage_height - 1);

                if (tile_top_left.x > view_lower_right.x || tile_top_left.y > view_lower_right.y
                    || tile_lower_right.x < view_top_left.x || tile_lower_right.y < view_top_left.y)
                {
                ...
                else
                {
                    ...
                    std::ptrdiff_t img_x0 = (tile_top_left.x >= view_top_left.x) ? tile_top_left.x
                                                                                 : view_top_left.x;
                    std::ptrdiff_t img_y0 = (tile_top_left.y >= view_top_left.y) ? tile_top_left.y
                                                                                 : view_top_left.y;

                    std::ptrdiff_t img_x1 = (tile_lower_right.x <= view_lower_right.x)
                                              ? tile_lower_right.x
                                              : view_lower_right.x;
                    std::ptrdiff_t img_y1 = (tile_lower_right.y <= view_lower_right.y)
                                              ? tile_lower_right.y
                                              : view_lower_right.y;
                    ...
                    BOOST_ASSERT(
                        tile_x0 < tile_width && tile_y0 < tile_height && tile_x1 < tile_width
                        && tile_y1 < tile_height);
                    ...
                    View dst_subimage_view = subimage_view(
                        dst_view,
                        (int)dst_x0,
                        (int)dst_y0,
                    ...
                    for (std::ptrdiff_t dst_row = 0; dst_row < dst_subimage_view.height();
                         ++dst_row)
                    {
                        ...
                        this->_cc_policy.read(begin, end, dst_subimage_view.row_begin(dst_row));
                    }  //for
                }
            }  // for
        }      // for
    ...
    template <typename Buffer, typename View>
    void read_tiled_data_full(const View& dst_view, int plane)
    {
        ///@todo: why is
        /// using row_buffer_helper_t = Buffer;
        /// not working? I get compiler error with MSVC10.
        ...
        for (unsigned int y = 0; y < image_height; y += tile_height)
        {
            for (unsigned int x = 0; x < image_width; x += tile_width)
            {
                ...
                View dst_subimage_view
                    = subimage_view(dst_view, x, y, current_tile_width, current_tile_length);
                ...
                for (int row = 0; row < dst_subimage_view.height(); ++row)
                {
                    ...
                    this->_cc_policy.read(begin, end, dst_subimage_view.row_begin(row));
                }  //for
            }      // for
        }          // for
    }
    ...
    template <typename Buffer, typename View>
    void read_stripped_data(const View& dst_view, int plane)
    {
        ...
        std::size_t size_to_allocate
            = buffer_size<typename View::value_type>(dst_view.width(), is_view_bit_aligned_t());
        ...
        skip_over_rows(row_buffer_helper.buffer(), plane);

        std::ptrdiff_t row = this->_settings._top_left.y;
        std::ptrdiff_t row_end = row + this->_settings._dim.y;
        std::ptrdiff_t dst_row = 0;

        for (; row < row_end; ++row, ++dst_row)
        {
            this->_io_dev.read_scanline(
                row_buffer_helper.buffer(), row, static_cast<tsample_t>(plane));

            this->_cc_policy.read(first, last, dst_view.row_begin(dst_row));
        }
    }
    ...
    )
    {
        ...
        std::size_t ret
            = std::max(width, ((scanline_size_in_bytes + element_size - 1) / element_size));
...
{
    tiff_type_format_checker(const image_read_info<tiff_tag>& info) : _info(info)
    {
    }
...
private:
    tiff_type_format_checker& operator=(const tiff_type_format_checker&)
    {
        return *this;
    }
...
{
    template <typename View>
    struct apply : public is_read_supported<typename get_pixel_type<View>::type, tiff_tag>
    {
    };
};
...
class dynamic_image_reader<Device, tiff_tag>
    : public reader<Device, tiff_tag, detail::read_and_no_convert>
{
    using parent_t = reader<Device, tiff_tag, detail::read_and_no_convert>;

public:
    dynamic_image_reader(const Device& io_dev, const image_read_settings<tiff_tag>& settings)
    ...
    void apply(any_image<Images...>& images)
    {
        ...
        else
        {
            ...
            detail::dynamic_io_fnobj<detail::tiff_read_is_supported, parent_t> op(this);

            apply_operation(view(images), op);
        }
    }
};
