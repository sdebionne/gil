...
struct get_property_f<2>
{
    ...
    bool call_me(typename Property::type& vs, std::shared_ptr<TIFF>& file) const
    {
        ...
        {
            std::copy_n(
                static_cast<typename Property::type::const_pointer>(pointer),
                length,
...
template <>
struct set_property_f<1>
{
    // For single-valued properties
    template <typename Property>
    inline bool call_me(typename Property::type const& value, std::shared_ptr<TIFF>& file) const
    {
...
struct set_property_f<2>
{
    ...
    // )
    template <typename Property>
    inline bool call_me(typename Property::type const& values, std::shared_ptr<TIFF>& file) const
    {
...
public:
    ...
    {
        return get_property_f<mp11::mp_size<typename Property::arg_types>::value>()
    ...
    inline bool set_property(const typename Property::type& value)
    {
        ...
        return set_property_f<mp11::mp_size<typename Property::arg_types>::value>()
            .template call_me<Property>(value, _tiff_file);
    }
    ...
    {
        return TIFFDefaultStripSize(_tiff_file.get(), 0);
    }
    ...
    int get_field_defaulted(uint16_t*& red, uint16_t*& green, uint16_t*& blue)
    {
        return TIFFGetFieldDefaulted(_tiff_file.get(), TIFFTAG_COLORMAP, &red, &green, &blue);
    }

    template <typename Buffer>
    void read_scanline(Buffer& buffer, std::ptrdiff_t row, tsample_t plane)
    {
        io_error_if(
            TIFFReadScanline(
                _tiff_file.get(), reinterpret_cast<tdata_t>(&buffer.front()), (uint32)row, plane)
                == -1,
            "Read error.");
    }

    void read_scanline(byte_t* buffer, std::ptrdiff_t row, tsample_t plane)
    {
        io_error_if(
            TIFFReadScanline(_tiff_file.get(), reinterpret_cast<tdata_t>(buffer), (uint32)row, plane)
                == -1,
            "Read error.");
    }
    ...
    template <typename Buffer>
    void write_scaline(Buffer& buffer, uint32 row, tsample_t plane)
    {
        io_error_if(
            TIFFWriteScanline(_tiff_file.get(), &buffer.front(), row, plane) == -1, "Write error");
    }

    void write_scaline(byte_t* buffer, uint32 row, tsample_t plane)
    {
        io_error_if(TIFFWriteScanline(_tiff_file.get(), buffer, row, plane) == -1, "Write error");
    }

    template <typename Buffer>
    void write_tile(Buffer& buffer, uint32 x, uint32 y, uint32 z, tsample_t plane)
    {
        if (TIFFWriteTile(_tiff_file.get(), &buffer.front(), x, y, z, plane) == -1)
        {
            std::ostringstream oss;
    ...
    {
        io_error_if(TIFFSetDirectory(_tiff_file.get(), directory) != 1, "Failing to set directory");
    }
    ...
    bool check_tile_size(
        tiff_tile_width::type& width,
    ...
    )
    {
        ...
        TIFFDefaultTileSize(_tiff_file.get(), &tw, &th);
...
public:
    ...
    file_stream_device(std::string const& file_name, read_tag)
    {
        ...
        io_error_if(
            (tiff = TIFFOpen(file_name.c_str(), "r")) == nullptr,
    ...
    file_stream_device(std::string const& file_name, write_tag)
    {
        ...
        io_error_if(
            (tiff = TIFFOpen(file_name.c_str(), "w")) == nullptr,
            "file_stream_device: failed to open file");

        _tiff_file = tiff_file_t(tiff, TIFFClose);
    }

    file_stream_device(TIFF* tiff_file) : tiff_device_base(tiff_file)
    {
    }
};
...
public:
    ostream_device(std::ostream& out) : _out(out)
    {
        ...
        io_error_if(
            (tiff = TIFFStreamOpen("", &_out)) == nullptr, "ostream_device: failed to stream");

        _tiff_file = tiff_file_t(tiff, TIFFClose);
    }

private:
    ostream_device& operator=(const ostream_device&)
    {
        return *this;
    }
...
public:
    istream_device(std::istream& in) : _in(in)
    {
        ...
        io_error_if(
            (tiff = TIFFStreamOpen("", &_in)) == nullptr, "istream_device: failed to stream");

        _tiff_file = tiff_file_t(tiff, TIFFClose);
    }

private:
    istream_device& operator=(const istream_device&)
    {
        return *this;
    }
...
template <>
struct photometric_interpretation<gray_t> : std::integral_constant<int, PHOTOMETRIC_MINISBLACK>
{
};
template <>
struct photometric_interpretation<rgb_t> : std::integral_constant<int, PHOTOMETRIC_RGB>
{
};
template <>
struct photometric_interpretation<rgba_t> : std::integral_constant<int, PHOTOMETRIC_RGB>
{
};
template <>
struct photometric_interpretation<cmyk_t> : std::integral_constant<int, PHOTOMETRIC_SEPARATED>
{
};
