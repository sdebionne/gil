...
public:
    using type = typename iterator_type_from_pixel<pixel_t, false, false, true>::type;
};


template <typename Channel, typename Layout, bool Mutable>
struct my_interleaved_pixel_iterator_type_from_pixel_reference<
...
struct tiff_write_is_supported
{
    ...
    struct apply : public is_write_supported<typename get_pixel_type<View>::type, tiff_tag>
    {
    };
};
...
class writer<Device, tiff_tag, Log> : public writer_backend<Device, tiff_tag>
{
private:
    using backend_t = writer_backend<Device, tiff_tag>;

public:
    writer(const Device& io_dev, const image_write_info<tiff_tag>& info) : backend_t(io_dev, info)
    {
    }
...
private:
    ...
    void write_view(const View& view)
    {
        ...
        tiff_bits_per_sample::type bits_per_sample
            = detail::unsigned_integral_num_bits<channel_t>::value;
        ...
        else
        {
            ...
            write_tiled_data(view, tw, th, typename is_bit_aligned<pixel_t>::type());
        }
    }
    ...
    template <typename View>
    void write_bit_aligned_view_to_dev(
        const View& view,
    ...
    {
        std::vector<
            pixel<typename channel_type<View>::type, layout<typename color_space_type<View>::type>>>
            row(view.size());
    ...
    )
    {
        ...
        using x_iterator = typename detail::my_interleaved_pixel_iterator_type_from_pixel_reference<
            typename View::reference>::type;
    ...
    {
        std::copy(view.begin(), view.end(), it);
    }
    ...
    template <typename View, typename IteratorType>
    void internal_write_tiled_data(
        const View& view,
        tiff_tile_width::type tw,
        tiff_tile_length::type th,
        byte_vector_t& row,
        ...
        while (i < view.height())
        {
            while (j < view.width())
            {
                if (j + tw < view.width() && i + th < view.height())
                {
                    ...
                    tile_subimage_view = subimage_view(
                        view,
                        static_cast<int>(j),
                        static_cast<int>(i),
                        static_cast<int>(tw),
                ...
                else
                {
                    ...
                    {
                        std::copy(
                            tile_subimage_view.row_begin(y), tile_subimage_view.row_end(y), it);
                        std::advance(it, tw);
                    }
                ...
                this->_io_dev.write_tile(row, static_cast<uint32>(j), static_cast<uint32>(i), 0, 0);
                j += tw;
            }
            j = 0;
            i += th;
...
class dynamic_image_writer<Device, tiff_tag> : public writer<Device, tiff_tag>
{
    using parent_t = writer<Device, tiff_tag>;

public:
    dynamic_image_writer(const Device& io_dev, const image_write_info<tiff_tag>& info)
    ...
    {
        detail::dynamic_io_fnobj<detail::tiff_write_is_supported, parent_t> op(this);
