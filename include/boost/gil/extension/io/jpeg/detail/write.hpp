...
struct jpeg_write_is_supported
{
    ...
    struct apply : public is_write_supported<typename get_pixel_type<View>::type, jpeg_tag>
    {
    };
};
...
public:
    writer(const Device& io_dev, const image_write_info<jpeg_tag>& info) : backend_t(io_dev, info)
    {
    }
...
private:
    ...
    void write_rows(const View& view)
    {
        ...
        this->get()->in_color_space = detail::
            jpeg_write_support<channel_t, typename color_space_type<View>::type>::_color_space;

        jpeg_set_defaults(this->get());

        jpeg_set_quality(this->get(), this->_info._quality, TRUE);
        ...
        jpeg_start_compress(this->get(), TRUE);
...
public:
    dynamic_image_writer(const Device& io_dev, const image_write_info<jpeg_tag>& info)
    ...
    {
        detail::dynamic_io_fnobj<detail::jpeg_write_is_supported, parent_t> op(this);
