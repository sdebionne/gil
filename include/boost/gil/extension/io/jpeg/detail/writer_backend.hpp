...
protected:
    ...
    /// Default Constructor
    ///
    jpeg_compress_wrapper() : _jpeg_compress_ptr(new jpeg_compress_struct(), jpeg_compress_deleter)
    {
    }

    jpeg_compress_struct* get()
    {
        return _jpeg_compress_ptr.get();
    }
    const jpeg_compress_struct* get() const
    {
        return _jpeg_compress_ptr.get();
    }
...
struct writer_backend<Device, jpeg_tag>
    : public jpeg_io_base
...
public:
    ...
    /// Constructor
    ///
    writer_backend(const Device& io_dev, const image_write_info<jpeg_tag>& info)
        : _io_dev(io_dev)
        , _info(info)
    {
        ...
        _dest._jdest.empty_output_buffer = reinterpret_cast<boolean (*)(j_compress_ptr)>(
...
protected:
    struct gil_jpeg_destination_mgr
    {
        ...
        writer_backend<Device, jpeg_tag>* _this;
    };
    ...
    {
        writer_backend<Device, jpeg_tag>::empty_buffer(cinfo);
    ...
    static void error_exit(j_common_ptr cinfo)
    {
        writer_backend<Device, jpeg_tag>* mgr
            = reinterpret_cast<writer_backend<Device, jpeg_tag>*>(cinfo->client_data);
