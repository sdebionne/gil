...
class reader<Device, jpeg_tag, ConversionPolicy>
    : public reader_base<jpeg_tag, ConversionPolicy>
...
public:
    ...
    // Constructor
    //
    reader(const Device& io_dev, const image_read_settings<jpeg_tag>& settings)
        : reader_base<jpeg_tag, ConversionPolicy>()
    ...
    reader(
        ...
        : reader_base<jpeg_tag, ConversionPolicy>(cc)
        , backend_t(io_dev, settings)
    {
    }
    ...
    void apply(const View& view)
    {
        ...
        default:
        {
            io_error("Unsupported jpeg color space.");
        }
        }
...
private:
    ...
    void read_rows(const View& view)
    {
        ...
        {
            io_error_if(
                jpeg_read_scanlines(this->get(), &row_adr, 1) != 1,
...
{
    jpeg_type_format_checker(jpeg_color_space::type color_space) : _color_space(color_space)
    {
    }
    ...
    bool apply()
    {
        return is_read_supported<typename get_pixel_type<typename Image::view_t>::type, jpeg_tag>::
                   _color_space
            == _color_space;
    }
...
{
    template <typename View>
    struct apply : public is_read_supported<typename get_pixel_type<View>::type, jpeg_tag>
    {
    };
};
...
class dynamic_image_reader<Device, jpeg_tag>
    : public reader<Device, jpeg_tag, detail::read_and_no_convert>
{
    using parent_t = reader<Device, jpeg_tag, detail::read_and_no_convert>;

public:
    dynamic_image_reader(const Device& io_dev, const image_read_settings<jpeg_tag>& settings)
    ...
    void apply(any_image<Images...>& images)
    {
        detail::jpeg_type_format_checker format_checker(
            this->_info._color_space != JCS_YCbCr ? this->_info._color_space : JCS_RGB);
        ...
        else
        {
            ...
            detail::dynamic_io_fnobj<detail::jpeg_read_is_supported, parent_t> op(this);
