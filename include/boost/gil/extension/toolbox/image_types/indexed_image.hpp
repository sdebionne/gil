...
struct indexed_image_deref_fn_base
{
    ...
    indexed_image_deref_fn_base(
        const indices_locator_t& indices_loc,
...
template <
    ...
    >
struct indexed_image_deref_fn : indexed_image_deref_fn_base<IndicesLoc, PaletteLoc>
{
    ...
    indexed_image_deref_fn(
        const typename base_t::indices_locator_t& indices_loc,
...
struct indexed_image_deref_fn<
    IndicesLoc,
    PaletteLoc,
    typename std::enable_if<detail::is_channel_integral<typename IndicesLoc::value_type>::value>::type>
...
{
    using type = virtual_2d_locator<indexed_image_deref_fn<IndicesLoc, PaletteLoc>, false>;
};
...
public:
    ...
    indexed_image_view() : image_view<Locator>(), _num_colors(0)
    {
    }
    ...
    indexed_image_view(const IndexedView& iv) : image_view<Locator>(iv)
...
indexed_image_view<typename indexed_image_locator_type<
    typename Index_View::locator,
    typename Palette_View::locator>::type>
    view(Index_View iv, Palette_View pv)
{
    using view_t = indexed_image_view<typename indexed_image_locator_type<
        typename Index_View::locator,
        typename Palette_View::locator>::type>;

    using defer_fn_t
        = indexed_image_deref_fn<typename Index_View::locator, typename Palette_View::locator>;

    return view_t(
        iv.dimensions(),
        pv.dimensions().x,
        typename view_t::locator(
            point_t(0, 0), point_t(1, 1), defer_fn_t(iv.xy_at(0, 0), pv.xy_at(0, 0))));
}
...
public:
    ...
    using locator_t =
        typename indexed_image_locator_type<indices_locator_t, palette_locator_t>::type;
    ...
    indexed_image(const indexed_image& img) : _indices(img._indices), _palette(img._palette)
    {
    }
...
private:
    ...
    {
        using defer_fn_t = indexed_image_deref_fn<indices_locator_t, palette_locator_t>;

        defer_fn_t deref_fn(view(_indices).xy_at(0, 0), view(_palette).xy_at(0, 0));
        ...
        _view = view_t(dimensions, num_colors, locator);
    }
...
inline const typename indexed_image<Index, Pixel>::view_t& view(indexed_image<Index, Pixel>& img)
{
    return img._view;
}

template <typename Index, typename Pixel>
inline const typename indexed_image<Index, Pixel>::const_view_t const_view(
    indexed_image<Index, Pixel>& img)
{
