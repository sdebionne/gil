...
public:
    ...
    file_stream_device(const char* file_name, read_tag = read_tag())
    {
        ...
        io_error_if(
            (file = fopen(file_name, "rb")) == nullptr,
    ...
    file_stream_device(const char* file_name, write_tag)
    {
        ...
        io_error_if(
            (file = fopen(file_name, "wb")) == nullptr,
    ...
    char getc()
    {
        ...
        io_error_if((ch = std::getc(get())) == EOF, "file_stream_device: unexpected EOF");
    ...
    {
        io_error_if(fseek(get(), count, whence) != 0, "file_stream_device: file seek error");
    }
    ...
    {
        std::size_t num_elements = fwrite(line.c_str(), sizeof(char), line.size(), get());

        io_error_if(num_elements < line.size(), "file_stream_device: line print error");
    }
...
public:
    ...
    std::size_t read(byte_t* data, std::size_t count)
    {
        ...
        do
        {
            _in.peek();
            std::streamsize c
                = _in.readsome(reinterpret_cast<char*>(data), static_cast<std::streamsize>(count));
...
public:
    ...
    {
        _out.write(reinterpret_cast<char const*>(data), static_cast<std::streamsize>(count));
    }
...
struct is_input_device : std::false_type
{
};
template <typename FormatTag>
...
struct is_adaptable_input_device<
    ...
    typename std::enable_if<
        mp11::mp_or<std::is_base_of<std::istream, T>, std::is_same<std::istream, T>>::value>::type>
    : std::true_type
{
...
struct is_read_device<
    ...
    T,
    typename std::enable_if<
        mp11::mp_or<is_input_device<FormatTag>, is_adaptable_input_device<FormatTag, T>>::value>::type>
    : std::true_type
{
};
 ...
 */
template <typename IODevice>
struct is_output_device : std::false_type
{
};
...
struct is_adaptable_output_device<
    ...
    T,
    typename std::enable_if<
        mp11::mp_or<std::is_base_of<std::ostream, T>, std::is_same<std::ostream, T>>::value>::type>
    : std::true_type
{
...
struct is_write_device<
    ...
    T,
    typename std::enable_if<
        mp11::mp_or<is_output_device<FormatTag>, is_adaptable_output_device<FormatTag, T>>::value>::
        type> : std::true_type
{
};
...
template <typename Device, typename FormatTag>
struct is_dynamic_image_reader<dynamic_image_reader<Device, FormatTag>> : std::true_type
{
};
...
template <typename Device, typename FormatTag>
struct is_dynamic_image_writer<dynamic_image_writer<Device, FormatTag>> : std::true_type
{
};
