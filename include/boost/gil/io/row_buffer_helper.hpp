...
template <typename Pixel, typename DummyT = void>
...
struct row_buffer_helper<Pixel, typename std::enable_if<is_bit_aligned<Pixel>::value>::type>
{
        ...
        typename is_homogeneous<Pixel>::type>::value>>
{
    ...
    row_buffer_helper(std::size_t width, bool in_bytes)
        : _c(
            (width * num_channels<pixel_type>::value * channel_type<pixel_type>::type::num_bits)
            >> 3)

        , _r(width * num_channels<pixel_type>::value * channel_type<pixel_type>::type::num_bits
             - (_c << 3))
    {
        if (in_bytes)
        {
...
struct row_buffer_helper_view<
    View,
    typename std::enable_if<is_bit_aligned<typename View::value_type>::value>::type>
