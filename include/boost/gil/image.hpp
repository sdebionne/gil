...
public:
    ...
    explicit image(std::size_t alignment = 0, const Alloc alloc_in = Alloc())
        : _memory(nullptr)
    ...
    template <
        typename Loc,
        typename std::enable_if<pixels_are_compatible<typename Loc::value_type, Pixel>::value, int>::type
        = 0>
...
private:
    ...
    using choose_pocma = typename std::conditional<
        ...
        std::true_type,
        typename std::allocator_traits<Alloc2>::propagate_on_container_move_assignment::type>::type;
...
private:
    ...
    std::size_t is_planar_impl(std::size_t const size_in_units, std::size_t const, std::false_type)
        const
    {
    ...
    std::size_t total_allocated_size_in_bytes(point_t const& dimensions) const
    {
        ...
        constexpr std::size_t _channels_in_image = std::conditional<
            is_pixel<value_type>::value,
            num_channels<view_t>,
            std::integral_constant<std::size_t, 1>>::type::value;
    ...
    {  // number of units per row
        ...
        if (_align_in_bytes > 0)
        {
            std::size_t alignment_in_memunits
                = _align_in_bytes * byte_to_memunit<typename view_t::x_iterator>::value;
    ...
    void create_view(point_t const& dims, std::false_type)  // is planar
    {
        ...
        _view = view_t(
            dims,
            typename view_t::locator(
                typename view_t::x_iterator(tmp), get_row_size_in_memunits(dims.x)));
...
template <typename Pixel, bool IsPlanar, typename Alloc>
inline const typename image<Pixel, IsPlanar, Alloc>::view_t& view(image<Pixel, IsPlanar, Alloc>& img)
{
    return img._view;
}
...
template <typename Pixel, bool IsPlanar, typename Alloc>
inline const typename image<Pixel, IsPlanar, Alloc>::const_view_t const_view(
    const image<Pixel, IsPlanar, Alloc>& img)
{
