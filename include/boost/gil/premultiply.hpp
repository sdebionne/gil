...
struct premultiply
{
    ...
    void operator()(const SrcP& src, DstP& dst) const
    {
        ...
        using has_alpha_t
            = std::integral_constant<bool, mp11::mp_contains<dst_colour_space_t, alpha_t>::value>;
...
template <
    typename SrcConstRefP,  // const reference to the source pixel
...
private:
    using src_pix_ref =
        typename SrcView::const_t::reference;  // const reference to pixel in SrcView
    using deref_t = premultiply_deref_fn<
        src_pix_ref,
        DstP>;  // the dereference adaptor that performs color conversion
...
public:
    ...
    static type make(const SrcView& sv)
    {
...
inline typename premultiplied_view_type<View, DstP>::type premultiply_view(const View& src)
{
