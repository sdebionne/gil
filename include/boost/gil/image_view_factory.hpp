...
struct dynamic_xy_step_type : dynamic_y_step_type<typename dynamic_x_step_type<View>::type>
{
};
...
/// \ingroup ImageViewTransformations
template <typename View>
struct dynamic_xy_step_transposed_type : dynamic_xy_step_type<typename transposed_type<View>::type>
{
};
...
typename type_from_x_iterator<Iterator>::view_t interleaved_view(
    std::size_t width,
    std::size_t height,
    Iterator pixels,
...
/// \brief Constructing image views from raw interleaved pixel data
template <typename Iterator>
auto interleaved_view(point<std::ptrdiff_t> dim, Iterator pixels, std::ptrdiff_t rowsize_in_bytes)
...
template <typename View>
struct channel_pointer_type : public channel_pointer_type_impl<View, view_is_mutable<View>::value>
{
};
}  // namespace detail
...
typename detail::channel_pointer_type<HomogeneousView>::type interleaved_view_get_raw_data(
    const HomogeneousView& view)
{
...
template <typename HomogeneousView>
typename detail::channel_pointer_type<HomogeneousView>::type planar_view_get_raw_data(
    const HomogeneousView& view,
    int plane_index)
{
    static_assert(is_planar<HomogeneousView>::value && view_is_basic<HomogeneousView>::value, "");
...
template <
    ...
    typename DstP,
    typename CC
    = default_color_converter>  // const_reference to the source pixel and destination pixel value
class color_convert_deref_fn
    : public deref_base<
          color_convert_deref_fn<SrcConstRefP, DstP, CC>,
          DstP,
          DstP,
          const DstP&,
          SrcConstRefP,
...
struct color_converted_view_type
    : public detail::_color_converted_view_type<SrcView, CC, DstP, typename SrcView::value_type>
{
...
inline typename color_converted_view_type<View, DstP, CC>::type color_converted_view(
    const View& src,
    CC cc)
{
...
inline typename color_converted_view_type<View, DstP>::type color_converted_view(const View& src)
{
...
inline typename dynamic_x_step_type<View>::type flipped_left_right_view(const View& src)
{
    ...
    return RView(
        src.dimensions(), typename RView::xy_locator(src.xy_at(src.width() - 1, 0), -1, 1));
}
...
inline typename dynamic_xy_step_transposed_type<View>::type transposed_view(const View& src)
{
    ...
    return RView(
        src.height(), src.width(), typename RView::xy_locator(src.xy_at(0, 0), 1, 1, true));
}
...
inline typename dynamic_xy_step_transposed_type<View>::type rotated90cw_view(const View& src)
{
    ...
    return RView(
        src.height(),
        src.width(),
        typename RView::xy_locator(src.xy_at(0, src.height() - 1), -1, 1, true));
}
...
inline typename dynamic_xy_step_transposed_type<View>::type rotated90ccw_view(const View& src)
{
    ...
    return RView(
        src.height(),
        src.width(),
        typename RView::xy_locator(src.xy_at(src.width() - 1, 0), 1, -1, true));
}
...
template <typename View>
inline auto subsampled_view(
    View const& src,
    typename View::coord_t x_step,
    typename View::coord_t y_step) -> typename dynamic_xy_step_type<View>::type
{
...
namespace detail {
template <typename View, bool AreChannelsTogether>
...
{
    using type = typename view_type<
        typename channel_type<View>::type,
        gray_layout_t,
        false,
        true,
        view_is_mutable<View>::value>::type;
...
struct __nth_channel_view_basic<View, true>
{
    ...
    static type make(const View& src, int n)
    {
        ...
        return interleaved_view(
            src.width(), src.height(), (x_iterator_t) & (src(0, 0)[n]), src.pixels().row_size());
    }
};
...
template <
    typename SrcP>  // SrcP is a reference to PixelConcept (could be pixel value or const/non-const reference)
...
public:
    ...
    using const_reference =
        typename pixel_reference_type<channel_t, gray_layout_t, false, false>::type;
...
namespace detail {
template <int K, typename View, bool AreChannelsTogether>
