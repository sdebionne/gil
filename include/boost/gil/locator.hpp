...
template <std::size_t D, typename Loc>
class locator_axis;
}  // namespace detail
...
template <
    typename Loc,
    typename XIterator,
    typename YIterator>  // The concrete subclass, the X-iterator and the Y-iterator
class pixel_2d_locator_base
{
public:
    ...
    using reference =
        typename std::iterator_traits<x_iterator>::reference;  // result of dereferencing
    using coord_t = typename std::iterator_traits<
        x_iterator>::difference_type;        // 1D difference type (same for all dimensions)
    ...
    template <std::size_t D>
    typename axis<D>::iterator const& axis_iterator() const
    {
        return detail::locator_axis<D, Loc>()(concrete());
    }
    template <std::size_t D>
...
private:
    ...
    const Loc& concrete() const
    {
        return (const Loc&)*this;
    }
...
public:
    ...
    inline iterator& operator()(Loc& loc) const
    {
        return loc.x();
    }
    inline iterator const& operator()(const Loc& loc) const
    {
        return loc.x();
    }
    inline iterator operator()(Loc& loc, const point_t& d) const
    {
        return loc.x_at(d);
    }
    inline iterator operator()(const Loc& loc, const point_t& d) const
    {
        return loc.x_at(d);
    }
};
...
public:
    using parent_t = pixel_2d_locator_base<
        ...
        typename iterator_adaptor_get_base<StepIterator>::type,
        StepIterator>;
    using const_t = memory_based_2d_locator<typename const_iterator_type<
        StepIterator>::type>;  // same as this type, but over const values
    ...
    {
        using type
            = memory_based_2d_locator<typename iterator_add_deref<StepIterator, Deref>::type>;
    ...
    memory_based_2d_locator(const memory_based_2d_locator<SI>& loc, coord_t y_step)
        : _p(loc.x(), loc.row_size() * y_step)
    {
    }
    template <typename SI>
    memory_based_2d_locator(
        const memory_based_2d_locator<SI>& loc,
        coord_t x_step,
        coord_t y_step,
        bool transpose = false)
        : _p(
            make_step_iterator(loc.x(), (transpose ? loc.row_size() : loc.pixel_size()) * x_step),
    ...
    {
        memunit_advance(x(), offset(d.x, d.y));
        return *this;
    }
    this_t& operator-=(const difference_type& d)
    {
    ...
    using cached_location_t = std::
        ptrdiff_t;  // type used to store relative location (to allow for more efficient repeated access)
    cached_location_t cache_location(const difference_type& d) const
    {
    ...
    }  // distance in mem units (bytes or bits) between adjacent rows
    std::ptrdiff_t pixel_size() const
    {
        return memunit_step(x());
    }  // distance in mem units (bytes or bits) between adjacent pixels on the same row
...
private:
    ...
    std::ptrdiff_t offset(x_coord_t x, y_coord_t y) const
    {
